...
def get_sun_pos(day, hour, min, sec, lat, long):
    ...
    azimuth = pi - arccos(
        (sin(lat) * cos(zenith) - sin(decl)) / (cos(lat) * sin(zenith))
    )
...
def sphere2xy(param, a, z):
    x = int(
        param["fullWidth"] / 360 * (a * 180 / pi - param["heading"])
        + param["img_width"] // 2
        + param["xoffs"]
    )
    return (x + param["xstart"]) % param["fullWidth"] - param["xstart"], int(
        z / pi * param["fullHeight"] - param["ystart"] + param["yoffs"]
    )
...
def sphere2xy_vect(param, a, z):
    ...
    return numpy.uint32((x + param["xstart"]) % param["fullWidth"]) - param[
        "xstart"
    ], numpy.uint32(z / pi * param["fullHeight"] - param["ystart"] + param["yoffs"])
...
def get_azel(coords):
    ...
    x, y, z = coords
    return (
        numpy.arctan2(x, y) % (2 * pi),
        numpy.arctan2(numpy.linalg.norm([y, x]), z),
...
def get_best_position(points, R1, R2, a=None, b=None):
    ...
    if b is not None:
        ...
        if abs(arcsin(s[2])) <= rng:
            ...
            aopt = a or arcsin(s[2] / numpy.linalg.norm(R1[2, :2])) - arctan(
                R1[2, 0] / R1[2, 1]
            )
    ...
    return (
        aopt,
        bopt,
        numpy.dot(rot(bopt) @ R1 @ rot(aopt) @ [1, 0, 0], s) * numpy.linalg.norm(ps),
    )
...
def get_best_btrack(points, R1, R2, a=None):
    ...
    if a is not None:
        ...
        #   take a derivative
        #   solve for a
        a = arctan2(R1[2, 1], R1[2, 0]) + arccos(
            srv / (numpy.hypot(srv, crv) * numpy.hypot(R1[2, 1], R1[2, 0]))
        )
...
def process_image(fobj, user_param):
    ...
    if (
        "GPS Latitude" in exif
        and "GPS Longitude" in exif
        and not user_param.get("ignore_exif_coords", False)
    ):
        ...
        if "latitude" not in user_param or "longitude" not in user_param:
            print(
                "If GPS coordinates are not included in image EXIF data, "
    ...
    img_color = cv2.imdecode(
        numpy.asarray(bytearray(cont), dtype=numpy.uint8), cv2.IMREAD_UNCHANGED
    )
    ...
    print(
        "SSE: {}, RMSE: {}, condition number: {}".format(r[1], rmse, r[3][0] / r[3][-1])
    )
    r = r[0]
    scale = 360 * 15 / param["fullWidth"]
    small_width = int(360 * 15 * width / param["fullWidth"])
    small_height = int(360 * 15 * height / param["fullWidth"])
    img_color_small = cv2.resize(
        img_color, (small_width, small_height), interpolation=cv2.INTER_NEAREST
    )
    diff = numpy.uint8(100 * numpy.abs(img_color_small @ r - 1))

    cv2.imwrite(f"{config.cachedir}/diff.jpg", diff)
    ...
    mask[1:-1, 1:-1] = cv2.resize(
        numpy.where(thr == 128, 1, 0),
        img_color.shape[1::-1],
    ...
    offs = 0
    if (
        "Date/Time Original" in exif
        and "Offset Time" in exif
        and user_param.get("interactive", True)
    ):
        d = datetime.datetime.strptime(
            exif["Date/Time Original"][:19], "%Y:%m:%d %H:%M:%S"
        ).timetuple()
    ...
    cv2.line(
        img,
        (0, param["fullHeight"] // 2 - param["ystart"]),
        (width, param["fullHeight"] // 2 - param["ystart"]),
        (0, 0, 255),
        5,
    )
    ...
    valid_c = numpy.array(
        [
            ...
            sin(valid[1])
            * cos(valid[0])
            * valid[2],  # negative because azimuth angles are cw
            cos(valid[1]) * valid[2],
        ]
    )
    ...
    for iter in range(20):
        totals = [
            get_best_position([cs[pos[n + 1]] - cs[pos[n]]], R1, R2, a=a0, b=b0)
            for n in range(npos - 1)
    ...
    for i, t in zip(pos, totals):
        ...
        print(
            "        Date and position:          {} -> a: {}, b: {}".format(
                ts, t[0] * 180 / pi, t[1] * 180 / pi
            )
        )
    ...
    dn = numpy.arange(364)
    cl = numpy.concatenate(
        [
            [0] * pos[0],
            *[[i] * (pos[i] - pos[i - 1]) for i in range(1, npos)],
            [0] * (364 - pos[-1]),
        ]
    )
    a_val, b_val = [], []
    ...
    results = {}
    if not user_param.get("track_azimuth", False) and not user_param.get(
        "track_zenith", False
    ):
        f_best_ze, f_best_az, f_energy = get_best_position(
            valid_c.T, R1, R2, a=a0, b=b0
        )
        ...
        print(
            "    Total energy (kWh):             {} kWh/m^2/year".format(
                f_energy / 3600000
            )
        )
        print(
            "    Average power:                  {} W/m^2".format(
                f_energy / (365 * 24 * 3600)
            )
        )
        ...
        results["fixed"] = {"a": f_best_ze, "b": f_best_az, "energy": f_energy}
    ...
    print(
        "    Total energy (kWh):             {} kWh/m^2/year".format(h_energy / 3600000)
    )
    print(
        "    Average power:                  {} W/m^2".format(
            h_energy / (365 * 24 * 3600)
        )
    )
    results["btrack"] = {"a": h_best_ze, "energy": h_energy}

    pts = [
        sphere2xy(
            param,
            *get_azel(R2 @ rot(i * pi / 180) @ R1 @ rot(h_best_ze) @ [1, 0, 0])[:2],
        )
        for i in range(0, 360, 10)
    ]
    for i in range(36):
        if (
            0 <= pts[i][0] < width
            and 0 <= pts[i][1] < height
            and 0 <= pts[i - 1][0] < width
            and 0 <= pts[i - 1][1] < height
        ):
    ...
    cv2.imwrite(f"{config.cachedir}/final{img_hash}.jpg", img)
    if user_param.get("interactive", True):
        cv2.destroyAllWindows()
...
if __name__ == "__main__":
    ...
    parse.add_argument(
        "--groups", default=2, help="Number of groups to use for best-groups analysis"
    )
    parse.add_argument(
        "--dt", default=15, help="Time increment for checking sun position, in minutes"
    )
    args = parse.parse_args()

    user_param = {"interactive": True}
    if args.latitude is not None:
        user_param["latitude"] = float(args.latitude)
    if args.longitude is not None:
        user_param["longitude"] = float(args.longitude)
    if args.azimuth is not None:
        if args.azimuth == "track":
            user_param["track_azimuth"] = True
        else:
            user_param["azimuth"] = float(args.azimuth)
    if args.zenith is not None:
        if args.zenith == "track":
            user_param["track_zenith"] = True
        else:
            user_param["zenith"] = float(args.zenith)
    if args.coord is not None:
        c = args.coord.split(";")
        R1 = numpy.array([float(x) for x in c[0].split(",")]).reshape((3, 3))
        if not numpy.allclose(R1 @ R1.T, numpy.eye(3)):
            print("R1 must be orthogonal")
        R2 = numpy.array([float(x) for x in c[1].split(",")]).reshape((3, 3))
        if not numpy.allclose(R2 @ R2.T, numpy.eye(3)):
            print("R2 must be orthogonal")
        user_param["coord"] = (R1, R2)
    if args.dt is not None:
        user_param["dt"] = float(args.dt)
    if args.groups is not None:
        user_param["groups"] = int(args.groups)
